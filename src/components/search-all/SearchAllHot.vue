<template>
    <div v-if="show" class="hot pl-30 bg-fff border-bottom ">
        ...
        <div v-for="item in showList" class="search-all-hot game-n ps-r clearfix border-bottom">
            ...
            <router-link
                    :to="{name:'ChooseClass', query:{gid:item[0]?item[0].gameId:'', gname:item[0]?item[0].gameName:'',gameType:item[0]?item[0].gameType:null} }"
                    class="fl f32 color-000" v-text="item[0].gameName"></router-link>
            <i class="pa"></i>
            <router-link
                    :to="{name:'ChooseClass', query:{gid:item[1]?item[1].gameId:'', gname:item[1]?item[1].gameName:'', gameType:item[1]?item[1].gameType:null} }"
